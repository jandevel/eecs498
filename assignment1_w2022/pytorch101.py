...
def zero_row_min(x: Tensor) -> Tensor:
    ...
    # Replace "pass" statement with your code
    y = x.clone()
    val, idx_y = x.min(dim=1)
    idx_x = torch.arange(y.shape[0])
    y[idx_x, idx_y] = 0
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
...
def batched_matrix_multiply_loop(x: Tensor, y: Tensor) -> Tensor:
    ...
    for b in range(b_dim):
        b_layers.append(torch.mm(x[b], y[b]))
    z = torch.stack(b_layers)
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
    return z
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    # Replace "pass" statement with your code
    x_dim = x.shape[0]
    mean_col = x.sum(dim=0) / x_dim
    std_1_step = (x - mean_col)**2
    std_col = torch.sqrt(std_1_step.sum(dim=0) / (x_dim-1))
